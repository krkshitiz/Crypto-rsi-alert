...
- `.dockerignore`
- Docker image now includes `config.properties.example` as `/app/config.properties` fallback to avoid startup crash if Render does not mount one.
- A root-level `config.properties` with placeholder values is committed to prevent startup crashes in environments that still launch older code paths expecting this file.
...
### Merge conflict: should you accept incoming changes?
Yes — for files related to this fix (`Main.java`, `Dockerfile`, `Crypto Rsi/Dockerfile`, `README.md`), prefer **Accept Incoming Change** so the missing-config crash fixes are kept.
...
- Verify the deploy log prints `config.properties not found at ... Using environment variables / defaults.`; if you still see `FileInputStream` at `Main.java:15`, Render is still running an older commit/image.
